...
unsigned int	ray_shade(t_obj3d *obj, t_world *w, t_vec3 p, t_vec3 r)
{
	...
	while (l)
	{
		...
		if (ray_intersect(w, light->pos, v, &ptr) > 0.999)
			if (v3dot(obj->normal(obj, p, r, v), v) *
				v3dot(obj->normal(obj, p, r, v), r) > 0)
				if ((v.x = v3dot(v3dot(obj->normal(obj, p, r, v), r) > 0 ?
					v3minus(obj->normal(obj, p, r, v)) :
					obj->normal(obj, p, r, v), v3normalize(v3minus(v)))) > 0)
					c = lightadd(c,
