...
int f(void *raytracer)
{
	...
	while (++y < ptr->resy)
	{
		...
		while (++x < ptr->resx)
		{
			if ((x - 50 < ptr->resx / 2 && x + 50 > ptr->resx / 2) && (y - 50 < ptr->resy / 2 && y + 50 > ptr->resy / 2))
				ptr->img->imgdata[x + y * ptr->resx] =  val + cos((x + ptr->frame))/15 * 10000;
			else if ((x - 100 < ptr->resx / 2 && x + 100 > ptr->resx / 2) &&  y > -10 * x + ptr->resy / 2)
